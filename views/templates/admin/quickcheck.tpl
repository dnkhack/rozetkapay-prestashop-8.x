...
<div class="quickcheck panel">
    <div class="row">
        <div class="col-sm-4">
            ...
            {/if}
            {l s='SSL' mod='rozetkapay'}
        </div>
        <div class="col-sm-4">
            {if $quickcheck.Versioncheck == 1}
                {$quickcheck_img_status_1}
                {l s='You are using the latest version' mod='rozetkapay'}
            {else}
                {$quickcheck_img_status_0}
                {l s='Modul update available' mod='rozetkapay'}
            {/if}
        </div>
        <div class="col-sm-4">
            {if $quickcheck.KeysProvided == 1}
                {$quickcheck_img_status_1}
                {l s='Credentials validated' mod='rozetkapay'}
            {else}
                {$quickcheck_img_status_0}
                {l s='Credentials not validated' mod='rozetkapay'}
            {/if}
        </div>
        <div class="col-sm-4">
            {$quickcheck_img_info}
            {if $quickcheck.sandbox == 1}
                {l s='Sandbox enabled' mod='rozetkapay'}
            {else}
                {l s='Livemode enabled' mod='rozetkapay'}
            {/if}
        </div>
    </div>
</div>